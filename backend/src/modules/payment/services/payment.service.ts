    ...
    async confirmPayment(dto: PaymentConfirmDto) {
        ...
        if (session.status === TransactionStatus.APPROVED) {
            throw new ConflictException(
                'Esta transacción ya fue procesada exitosamente',
            );
        }
        if (session.status === TransactionStatus.REJECTED) {
            throw new BadRequestException(
                'Esta transacción fue rechazada previamente',
            );
        }
        ...
        if (session.expiresAt < now) {
            ...
            await this.transactionSessionDao.updateStatus(
                session.id,
                TransactionStatus.REJECTED,
            );
            throw new BadRequestException(
                'El token ha expirado. Por favor, solicite un nuevo pago.',
            );
        }
